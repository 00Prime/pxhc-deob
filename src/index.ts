...
function processCode(code: string) {
    ...
    CallExpression(path) {
      ...
      if (
        ...
        if (
          ...
          path.parentPath.node.callee.name == "parseInt"
        )
          return;
  ...
  traverse(ast, {
    CallExpression(path) {
      const { callee, arguments: args } = path.node;
      if (t.isSequenceExpression(args[0]) && args[0].expressions.length == 3) {
        if (t.isStringLiteral(args[0].expressions[2])) {
          path.replaceWith(t.stringLiteral(decode(args[0].expressions[2].value)));
          return;
        }
      }
      if (
      ...
      if (args) path.replaceWith(t.stringLiteral(decode(args[0].value)));
    },
  });
  ...
  traverse(ast, {
    FunctionDeclaration(path: NodePath<t.FunctionDeclaration>) {
      ...
      if (constant && !referenced) {
        console.log("removed function ", node.id.name);
  ...
  traverse(ast, {
    CallExpression(path) {
      ...
      if (!t.isIdentifier(callee)) return; // Verify that the callee is a identifer
      if (args.length !== 3) return;

      const functionDefinition = path.scope.getBinding(callee.name).path;



      //Make sure its the function we want
      let isObjectFunc = false; // Flag to track whether the condition is met
      functionDefinition.traverse({
        ReturnStatement(path: NodePath<t.ReturnStatement>) {
          if (t.isSequenceExpression(path.node.argument)) {
            isObjectFunc = true; // Set the flag if the condition is met
            path.stop(); // Stop traversing further
          }
        },
      });

      if (!isObjectFunc) return;



      // Replace the call expression based on the type of the first argument
      // if (t.isAssignmentExpression(args[0])) {
      ...
      //         t.objectExpression([
      //           //@ts-ignore
      //           t.objectProperty(t.identifier(args[1].value), args[2])
      //         ])
      //       )
      //     );
      //   } else {
      ...
      //     // debugger
      //     console.error('Unexpected type for args[1]');
      //   }
      ...
      //       t.assignmentExpression(
      //         '=',
      //         t.memberExpression(args[0], t.identifier(args[1].value), false),
      //         //@ts-ignore
      //         args[2]
      //       )
      //     );
      //   } else {
      //     console.error('Unexpected type for args[1] or args[2]');
      //   }
      ...
      if (t.isAssignmentExpression(args[0])) {
        //@ts-ignore
        handleReplacement(path, args[0].left, args[1], args[2], true);
      } else if (t.isIdentifier(args[0])) {
        //@ts-ignore
        handleReplacement(path, args[0], args[1], args[2], false);
      } else if (t.isObjectExpression(args[0])) {
        //@ts-ignore
        handleReplacement(path, null, args[1], args[2], false);
      }
    }
  })
...
function handleReplacement(
  ...
  if (t.isStringLiteral(property) && t.isExpression(value)) {
    const propName = property.value;
    const propKey = propName.includes('-')
      ...
      if (isAssignment) {
        path.replaceWith(
          t.assignmentExpression(
            ...
            left,
            t.objectExpression([
              t.objectProperty(propKey, value)
            ])
          )
        );
      } else {
        path.replaceWith(
          t.assignmentExpression(
            '=',
            //@ts-ignore
            t.memberExpression(left, propKey, propName.includes('-')),
            value
          )
        );
      }
...
async function main() {
  ...
  let code = '';
  if (fetchNew) {
    const url = 'https://captcha.px-cdn.net/PXO97ybH4J/captcha.js?a=c&m=0';
    code = await fetchCodeFromURL(url);
  }
  else {
    code = readFileSync("../out/captcha.js", "utf8");
  }
  processCode(code);
}
